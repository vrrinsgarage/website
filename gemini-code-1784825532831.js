...
/* ==========================================
   2. ALUR FLOW MODAL LAYANAN
   ========================================== */
let currentSystemKey = null;
let currentCategoryName = null;

// Fungsi pembantu sanitasi teks dari tanda petik
...
function openSystemFlow(systemKey) {
  ...
  for (let catName in sysData.categories) {
    ...
    const safeCatName = escapeQuotes(catName);
    html += `<div class="list-menu-item" onclick="renderServiceList('${safeCatName}')"><div><div class="item-title">${catName}</div><span class="item-badge">${count} Layanan</span></div><i class="fas fa-chevron-right text-primary"></i></div>`;
  }
  
  html += `</div>`;
  modalBody.innerHTML = html;
  modalBody.scrollTop = 0; // Otomatis scroll ke atas
...
function renderServiceList(catName) {
  ...
  if (!sysData || !sysData.categories[catName]) return;
...
function renderServiceDetail(index) {
  const sysData = servicesDatabase[currentSystemKey];
  if (!sysData || !sysData.categories[currentCategoryName]) return;

  const item = sysData.categories[currentCategoryName][index];
  if (!item) return;
...
function filterServices() {
  ...
  if (matches.length > 0) {
    ...
    resultsContainer.innerHTML = matches.map(m => {
      const safeCatName = escapeQuotes(m.catName);
      return `
        <div class="search-item" onclick="openDirectDetail('${m.sysKey}', '${safeCatName}', ${m.index})">
          <span>${m.name}</span>
